    ...
    <section className="space-y-6 animate-in fade-in duration-500">
      {(references || []).map((ref, i) => (
        <div key={i} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm space-y-4">
          ...
          <TextAreaGroup label="Contact / Details" value={ref.reference} onChange={(e: any) => updateField(`references.${i}.reference` as any, e.target.value)} placeholder="Manager at Acme Corp - jane@acme.com" />
        </div>
      ))}
